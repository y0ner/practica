...
import { SeasonService } from '../../../services/Temporada.service';
import { SeasonResponseI } from '../../../models/Temporada';

@Component({
  selector: 'app-Temporada-getall',
  standalone: true,
  imports: [
    CommonModule,
    RouterModule,
  ...
  loadData(): void {
    ...
    this.subscription.add(
      this.TemporadaService.getAll().subscribe({
        next: (data) => {
          this.Temporada = data;
          this.TemporadaService.updateLocalData(data);
          this.loading = false;
        },
        error: (error) => {
